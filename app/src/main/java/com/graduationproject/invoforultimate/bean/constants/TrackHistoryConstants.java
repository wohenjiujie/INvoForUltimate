package com.graduationproject.invoforultimate.bean.constants;

/**
 * Created by INvo
 * on 2020-02-12.
 */
public final class TrackHistoryConstants {

    public static  String TAG = "historyLog";
    public static  int GET_TRACK_HISTORY_INFO = 0x301;
    public static  int GET_TRACK_INTENT_INFO = 0x302;
    public static  int DELETE_TRACK_INFO = 0x303;
    public static  String UPGRADE_STATUS = "更新成功";
    public static  String DELETE_STATUS = "更新成功";
}
